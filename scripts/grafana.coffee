...
module.exports = (robot) ->
  ...
  robot.respond /(\w+) ([A-Za-z0-9\-\.]+)(.*)?/i, (msg) ->
    test = [
      {
        keyID: 'cpu',
        body: {
          dashboard: 'prometheus-system',
          panelID: 2
        }
      },
      {
    ...
    slug = msg.match[1] and  msg.match[1].trim()
    host = msg.match[2] and msg.match[2].trim()
    # remainder = msg.match[3].trim()
    timeFrom = if msg.match[3] then msg.match[3].trim()else '3h'
    timespan = {
      from: "now-#{timeFrom}"
      to: 'now'
    }
    variables = if host is "fl" then "&var-node=luxproject&var-node=luxproject-artifactory&var-node=luxproject-bamboo&var-node=luxproject-confluence&var-node=luxproject-crowd&var-node=luxproject-crucible&var-node=luxproject-db&var-node=luxproject-git&var-node=luxproject-jira&var-node=luxproject-sd&var-node=oro-lpbb-01&var-node=oro-lpdb-01&var-node=oro-lpmmapp-01&var-node=oro-lpmmdb-01&var-node=sen-core-02&var-node=sen-core-03&var-node=sen-core-04&var-node=sen-core-06&var-node=sen2db&var-node=sen2svn&var-node=sentinel2&var-device=All" else "&var-host=#{host}"
    template_params = []
    visualPanelId = false
    apiPanelId = false
    pname = false
    
    parts = test.filter (item) ->
      item.keyID == slug
    if parts.length > 0
      #slug = if host is 'fleet' then if slug is 'io' then 'fleet-disk-performance' else if slug is 'bw' then 'undefined' else parts[0].body.dashboard
      slug = if host == 'fleet' then (if slug == 'io' then 'fleet-disk-performance' else 'none') else if slug == 'bw' then 'undefined' else parts[0].body.dashboard
      visualPanelId =  if host == 'fleet' then 1 else parts[0].body.panelID
      ...
      callGrafana "dashboards/db/#{slug}", (dashboard) ->
        ...
        for row in data.rows
          for panel in row.panels
            ...
            if visualPanelId && visualPanelId != panel.id
              continue
            ...
            imageUrl = "#{grafana_host}/render/#{apiEndpoint}/db/#{slug}?panelId=#{visualPanelId}&width=1400&height=500#{variables}&from=#{timespan.from}&to=#{timespan.to}"
            link = "#{grafana_host}/dashboard/db/#{slug}?panelId=#{visualPanelId}&fullscreen#{variables}&from=#{timespan.from}&to=#{timespan.to}"
            imgTitle = title.toLowerCase().replace(' ','-')
  ...
  sendRobotResponse = (msg, title, image, link) ->
    switch robot.adapterName
      ...
      when 'hipchat'
        ...
        title2 = title.toLowerCase().replace(' ', '-') + '.png'
        targetName = if msg.envelope.room then "room/#{msg.envelope.user.reply_to.match(/_(\S+?)@/)[1]}" else "user/#{msg.envelope.user.reply_to.match(/_(\S+?)@/)[1]}"
        targetName = targetName.replace /_/g, '%20'
        request(image).pipe(fs.createWriteStream(title2)).on 'close', ->
            ...
            request.post {
              ...
              'multipart': [
                ...
                }
                {
                  ...
                  'Content-Disposition': 'attachment; name="file"; filename="' + title2 + '"'
                  'body': data
                }
              ]
            }, (err, results) ->
